...
with open(os.path.join(current_dir, "Dockerfile"), "w+") as docker_file:
    ...
    docker_file.writelines(["SHELL [\"/bin/bash\", \"-c\"]\n",
                           ...
                           "\n",
                           "RUN apt update && apt install -y build-essential clang-format-11 cmake cppcheck libcurl4-openssl-dev nlohmann-json3-dev python3-pip libfmt-dev libgtest-dev git\n",
    ...
    docker_file.write("RUN git config --global user.email \"%s\"\n" % user_email)
